...
# Use the repository root (folder containing this script) as the dataset root
ROOT = THIS.parent
...
RESULTS   = ROOT / "output_approach_1"
...
def main():
    ...
    # Save label mapping for downstream inference (original class id -> contiguous index)
    mapping_path = RESULTS / "label_mapping.json"
    with mapping_path.open("w", encoding="utf-8") as f:
        json.dump({str(k): int(v) for k, v in id2idx.items()}, f, indent=2)
