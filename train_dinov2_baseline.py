...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="outputs", type=str, help="Directory for outputs.")
    parser.add_argument(
        "--checkpoint-path",
...
def extract_embeddings(
    ...
    if not targets:
        return emb_tensor.numpy(), None

    first_item = targets[0]
    if torch.is_tensor(first_item):
        target_tensor = torch.cat(targets, dim=0)
        return emb_tensor.numpy(), target_tensor.numpy()

    # Assume metadata such as image paths (list of strings)
    return emb_tensor.numpy(), np.array(targets)
...
def main() -> None:
    ...
    train_embeddings, train_labels = extract_embeddings(backbone, train_loader, device)
    val_embeddings, val_labels = extract_embeddings(backbone, val_loader, device)
    test_embeddings, test_image_names = extract_embeddings(backbone, test_loader, device)
